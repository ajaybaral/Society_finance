...
<?php include 'navbar.php'; ?>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" id="maintenanceForm">
        ...
        <!-- Add other shop options -->
            <!-- Add options for other floors -->
        </select>

        <label for="sr_no">Receipe.no:</label>
        ...
        <label for="non-occ">Non occupancy Fees:100</label>
        <input type="checkbox" id="non-occ" name="non-occ" value="100" readonly>


        <label for="parkingFees">Parking Fees:</label>
        <input type="number" id="parkingFees" name="parkingFees">

        <label for="lateFees">Late Fees:50</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
    ...
    if ($checkResult->num_rows > 0) {
        ...
        $sinkingFees = 100;
        $parkingFees = $_POST["parkingFees"] ;
        $lateFees = isset($_POST["lateFees"]) ? 50 : 0;
        $nonoccupancy_Fees = isset($_POST["non-occ"]) ? 100 : 0;
        
        $finalAmount = $paymentAmount + $sinkingFees + $parkingFees + ($lateFees ? 25 : 0)+($nonoccupancy_Fees ? 100 : 0); // Calculate final amount

        // SQL query to insert data into the database
        $sql = "INSERT INTO maintenance_records (date, month_year, flat_no,receipt_no, payment_amount, sinking_fees, parking_fees, late_fees, final_amount)
                VALUES ('$date', '$monthYear', '$flatNo','$Receipt', '$paymentAmount', '$sinkingFees', '$parkingFees', '$lateFees', '$finalAmount')";
...
const parkingFeesInput = document.getElementById("parkingFees"); // Change variable name
...
const nonoccupancy_Fees = document.getElementById("non-occ");

// Function to calculate and update final amount
function updateFinalAmount() {
    let finalAmount = parseFloat(paymentAmountInput.value);

    if (parkingFeesInput.value) { // Check if parking fees value is not empty
        finalAmount += parseFloat(parkingFeesInput.value);
    }
...
parkingFeesInput.addEventListener("input", updateFinalAmount); // Change event listener
lateFeesCheckbox.addEventListener("change", updateFinalAmount);
paymentAmountInput.addEventListener("input", updateFinalAmount);
nonoccupancy_Fees.addEventListener("change", updateFinalAmount);
